...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (loginForm) loginForm.onsubmit = (e) => {
        e.preventDefault();
        if (!dbLoaded) return toast('⏳ Carregando banco de dados...');
...
function renderAdminUI() {
    ...
    if (taskList) taskList.innerHTML = tasks.length ? [...tasks].reverse().map(t => {
        ...
        <div class="task-card glass ${done ? 'completed' : ''}">
            ...
            ${t.employeeNote ? `<div class="admin-note-display">"${t.employeeNote}"</div>` : ''}
        </div>`;
...
function renderUserUI() {
    ...
    document.getElementById('user-cargo-badge').textContent = currentUser.cargo || 'Equipe';
    ...
    if (list) list.innerHTML = myTasks.length ? [...myTasks].reverse().map(t => {
        ...
        <div class="task-card glass ${done ? 'completed' : ''}">
            ...
            <div class="task-card-footer">
                ...
                ${done ? '<span class="status-text-done">✔</span>' : `<button class="btn-complete" onclick="completeTask('${t.id}')">Concluir</button>`}
            </div>
            <div class="task-notes-box">
                <textarea id="note-${t.id}" class="notes-textarea" placeholder="Nota..." ${done?'disabled':''}>${t.employeeNote || ''}</textarea>
...
async function completeTask(tid) {
    ...
    if (!t || !confirm(`Concluir missão?`)) return;
...
async function saveUserEdit(e) {
    ...
    const phFile = document.getElementById('edit-user-photo').files[0];
    const up = async (b64) => {
        const data = { name, cargo, pass, phone, role };
        if (b64) data.photo = b64;
        await updateDoc(doc(db, "users", id), data);
        document.getElementById('edit-user-modal').classList.add('hidden');
        toast('✅ Usuário Atualizado!');
    };
    if (phFile) { const rd = new FileReader(); rd.onload = (ev) => up(ev.target.result); rd.readAsDataURL(phFile); }
    else up();
}
